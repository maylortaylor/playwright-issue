    ...
    visitor: {
      ...
      ImportDeclaration(p) {
        ...
        for (const specifier of importNode.specifiers) {
          ...
          const {
            fullName
          } = componentInfo(specifier, importNode.source.value, this.filename, componentName);
      ...
      JSXElement(path) {
        ...
        let nameOrExpression = '';
        if (t.isJSXIdentifier(jsxName)) nameOrExpression = jsxName.name;else if (t.isJSXMemberExpression(jsxName) && t.isJSXIdentifier(jsxName.object) && t.isJSXIdentifier(jsxName.property)) nameOrExpression = jsxName.object.name + '.' + jsxName.property.name;
        if (!nameOrExpression) return;
  ...
  (0, _babelBundle.traverse)(node, {
    enter: p => {
      ...
      if (t.isJSXElement(p.node)) {
        ...
        if (t.isJSXMemberExpression(p.node.openingElement.name) && t.isJSXIdentifier(p.node.openingElement.name.object) && t.isJSXIdentifier(p.node.openingElement.name.property)) names.add(p.node.openingElement.name.object.name + '.' + p.node.openingElement.name.property.name);
      }
...
function componentInfo(specifier, importSource, filename, componentName) {
  const isModuleOrAlias = !importSource.startsWith('.');
  const unresolvedImportPath = _path.default.resolve(_path.default.dirname(filename), importSource);
  ...
  if (t.isImportDefaultSpecifier(specifier)) return {
    fullName: prefix + fullNameSuffix,
    importedNameProperty,
    deps: [],
    ...pathInfo
  };
  if (t.isIdentifier(specifier.imported)) return {
    fullName: prefix + '_' + specifier.imported.name + fullNameSuffix,
  ...
  return {
    fullName: prefix + '_' + specifier.imported.value + fullNameSuffix,
